-- ========================================================================================
-- ВСЕ НЕОБХОДИМЫЕ ФУНКЦИИ ДЛЯ РАБОТЫ САЙТА
-- Выполняйте в Supabase SQL Editor в ЭТОМ ПОРЯДКЕ
-- ========================================================================================
...
BEGIN
    ...
    SELECT json_build_object(
      ...
      ''user_reaction'', CASE
        WHEN $6 IS NOT NULL THEN (
          ...
          WHERE article_id = a.id AND user_id = $6
        )
        ELSE NULL
      END
    )
  ...
  EXECUTE query_text
  INTO result
  USING p_search, p_tags, p_difficulty, p_skip, p_take, user_uuid;
