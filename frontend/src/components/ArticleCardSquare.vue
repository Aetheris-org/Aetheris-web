<template>
  <div class="article-card-square" @click="handleCardClick">
    <!-- Preview Image - самый верхний элемент -->
    <div class="thumb" @click.stop="openImagePreview">
      <img 
        v-if="safePreviewUrl" 
        :src="safePreviewUrl"
        :alt="article.title"
      ...
      <div v-else class="thumb-fallback">
        <i class="pi pi-image thumb-icon"></i>
      </div>
    </div>
    
    <!-- Content Section -->
    <div class="info">
      <!-- Author and date - сверху -->
      <div class="meta">
      ...
      <!-- Metadata: difficulty, reading time, views -->
      <div class="metadata">
        <div class="metadata-item difficulty" :class="`difficulty-${article.difficulty || 'medium'}`">
          <img class="metadata-icon" :src="fireIcon" alt="Difficulty" width="12" height="12">
        ...
        <div class="metadata-item views">
          ...
          <span class="metadata-text">{{ article.views || 0 }}</span>
        </div>
      </div>

      <!-- Tags - горизонтально -->
      <div class="tags-container" v-if="article.tags && article.tags.length > 0">
        <Tag
        ...
        />
        <span v-if="article.tags.length > 3" class="tags-more">+{{ article.tags.length - 3 }}</span>
      </div>
    </div>

    <!-- Mini Footer with action buttons -->
    <div class="card-footer" @click.stop>
      <div class="action-group" v-if="article.status !== 'draft'">
        <button class="action-btn like-btn" @click.stop="onLike" :class="{ 'active': isLiked }" :disabled="isLikeLoading">
          <svg v-if="!isLikeLoading" class="heart-icon" :class="{ 'filled': isLiked }" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button class="action-btn dislike-btn" @click.stop="onDislike" :class="{ 'active': isDisliked }" :disabled="isDislikeLoading">
          <svg v-if="!isDislikeLoading" class="dislike-icon" :class="{ 'filled': isDisliked }" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button class="action-btn comment-btn" @click.stop="onComment">
          <svg class="comment-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button class="action-btn bookmark-btn" @click.stop="onBookmark" :class="{ 'active': isBookmarked }">
          <svg class="bookmark-icon" :class="{ 'filled': isBookmarked }" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button class="action-btn share-btn" @click.stop="onShare" :class="{ 'active': isShared }">
          <svg class="share-icon" :class="{ 'filled': isShared }" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import { sanitizePreviewUrl } from '@/utils/avatarValidation'

const fireIcon = new URL('@/assets/svgs/fire_ico.svg', import.meta.url).href

const props = defineProps<{ article: Article }>()
const emit = defineEmits(['authorClick','articleClick', 'tagClick', 'shareArticle'])
...
.self-badge {
  ...
  padding: 3px 8px;
  border-radius: 10px;
  background: var(--badge-self-bg);
  color: var(--badge-self-text);
  font-size: 12px;
  font-family: var(--font-sans);
  font-weight: 700;
