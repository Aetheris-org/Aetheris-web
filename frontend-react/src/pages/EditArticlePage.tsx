...
export default function EditArticlePage() {
          ...
          if (article) {
            ...
            if (article.content) {
              try {
                ...
                logger.error('[EditArticlePage] Failed to parse article content:', error)
                setContent('')
                setContentJSON(null)
              }
            }
            
            logger.debug('[EditArticlePage] Article loaded for editing:', { articleId: article.id })
          } else {
            toast({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      // Помечаем черновик как намеренно сохраненный и очищаем localStorage
      try {
        ...
        logger.warn('[EditArticlePage] Failed to set intentionally saved flag:', e)
      }
