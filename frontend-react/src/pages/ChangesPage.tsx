...
export default function ChangesPage() {
  ...
  const handleCreateArticle = () => {
    navigate('/create?category=changes')
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container max-w-4xl mx-auto space-y-6 sm:space-y-8 pb-4 sm:pb-6 pt-4 sm:pt-6 px-4 sm:px-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{t('header.changes')}</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {t('changes.description') || 'Recent changes and updates from administrators'}
            </p>
          </div>
          {isAdmin && (
            <Button onClick={handleCreateArticle} size="sm" className="gap-2">
              <PenSquare className="h-4 w-4" />
              <span className="hidden sm:inline">{t('changes.createArticle') || 'Create Article'}</span>
            </Button>
          )}
        </div>
        ...
        {isLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center px-4">
              <p className="text-lg font-medium">{t('changes.noArticles') || 'No changes yet'}</p>
              <p className="mt-2 text-sm text-muted-foreground">
                {t('changes.noArticlesDescription') || 'Check back later for updates and changes'}
              </p>
            </CardContent>
          </Card>
        ) : (
